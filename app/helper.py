...
def load_json_item(column: str, file_name='diseases_and_symptoms.json') -> list[str]:
    """
    ...
    # Get the directory of the app folder
    app_dir = os.path.dirname(os.path.abspath(__file__))
    # Construct the path to the JSON file outside the app folder
    file_path = os.path.join(app_dir, '..', file_name)
    with open(file_path, 'r') as f:
        data = json.load(f)
    return data[column]

def process_string_input(input: str, all_symptoms: list[str], client: Client) -> list[str]:
    """
    ...
    chat_completion = client.chat.completions.create(
        ...
        messages=[
            ...
            {
                "role": "system",
                "content": f"""You are an assistant who helps the hospital to map symptoms from a patient's story, a collection of known symptoms are as follows, {all_symptoms}
...
def get_majority_output(input: str, threshold: int, all_symptoms: list[str], client: Client) -> list[str]:
    """
    ...
    for i in range(threshold):
        llm_output_symptoms = process_string_input(input, all_symptoms, client)
    ...
    for symptom, count in symptom_count.items():
        ...
        if count >= minimum and symptom in all_symptoms:
